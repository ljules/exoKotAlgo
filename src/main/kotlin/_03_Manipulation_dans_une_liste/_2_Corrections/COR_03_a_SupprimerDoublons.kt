...
fun supprimerDoublons(liste: List<Int>): MutableList<Int> {
    ...
    while(i < listeCopie.lastIndex) {
        //Boucle de recherche des doublons de listeCopie[i] :
        var k = i
        while(k < listeCopie.lastIndex) {
            for (j in k + 1..listeCopie.lastIndex) {
                k = j
                if (listeCopie[i] == listeCopie[j]) {
                    listeCopie.removeAt(k)
                    break
                }
            }
        }
