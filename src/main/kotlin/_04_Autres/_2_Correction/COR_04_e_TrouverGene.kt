...
fun trouverGene(gene: String, seqADN: String): Boolean {
    val tailleGene = gene.length
    val tailleSeqADN = seqADN.length
    val finRecherche = tailleSeqADN - tailleGene
    // Boucle de recherche principale :
    for (i in 0..finRecherche) {
        // Boucle de parcours du gène :
        for (j in 0.. tailleGene - 1) {
            if (gene[j] != seqADN[i + j]) break
            if (j == tailleGene - 1) return true
        }
    }
    return false
}
